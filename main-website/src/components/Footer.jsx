import { Link } from 'react-router-dom';

const links = [
  ['Home',     '/'],
  ['About',    '/about'],
  ['Events',   '/events'],
  ['Team',     '/team'],
  ['Sponsors', '/sponsors'],
  ['Contact',  '/contact'],
];

export default function Footer() {
  return (
    <footer style={{
      background: '#0a0d0f',
      color: '#e8e4dc',
      padding: '50px 4rem 0',
      borderTop: '1px solid rgba(255,255,255,0.06)',
      fontFamily: "'DM Sans', sans-serif",
    }}>
      {/* Main 3-column grid */}
      <div style={{
        display: 'flex', gap: '3rem', flexWrap: 'wrap',
        justifyContent: 'space-between', marginBottom: '2.5rem',
      }}>

        {/* Left — Brand */}
        <div style={{ flex: '1 1 250px' }}>
          <div style={{
            fontFamily: "'Saman', serif",
            fontSize: '2.5rem', fontWeight: 400,
            color: '#e8e4dc', marginBottom: '0.4rem',
            letterSpacing: '0.04em',
          }}>Abhivriddhi</div>
          <div style={{
            fontSize: '0.82rem', color: 'rgba(232,228,220,0.45)',
            marginBottom: '1.5rem', letterSpacing: '0.02em',
          }}>Student Training &amp; Development Committee</div>
          <div style={{ display: 'flex', gap: '12px' }}>
            {/* LinkedIn */}
            <a href="#" aria-label="LinkedIn" style={{
              color: 'rgba(232,228,220,0.45)', width: 36, height: 36,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8,
              transition: 'color 0.3s, border-color 0.3s',
            }}
            onMouseEnter={e => { e.currentTarget.style.color='#b8cc8a'; e.currentTarget.style.borderColor='rgba(184,204,138,0.35)'; }}
            onMouseLeave={e => { e.currentTarget.style.color='rgba(232,228,220,0.45)'; e.currentTarget.style.borderColor='rgba(255,255,255,0.08)'; }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M20.5 2h-17A1.5 1.5 0 002 3.5v17A1.5 1.5 0 003.5 22h17a1.5 1.5 0 001.5-1.5v-17A1.5 1.5 0 0020.5 2zM8 19H5v-9h3zM6.5 8.25A1.75 1.75 0 118.3 10 1.78 1.78 0 016.5 8.25zM19 19h-3v-4.74c0-1.42-.6-1.93-1.38-1.93A1.74 1.74 0 0013 14.19V19h-3v-9h2.9v1.3a3.11 3.11 0 012.7-1.4c1.55 0 3.36.86 3.36 3.66z"/></svg>
            </a>
            {/* YouTube */}
            <a href="#" aria-label="YouTube" style={{
              color: 'rgba(232,228,220,0.45)', width: 36, height: 36,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8,
              transition: 'color 0.3s, border-color 0.3s',
            }}
            onMouseEnter={e => { e.currentTarget.style.color='#b8cc8a'; e.currentTarget.style.borderColor='rgba(184,204,138,0.35)'; }}
            onMouseLeave={e => { e.currentTarget.style.color='rgba(232,228,220,0.45)'; e.currentTarget.style.borderColor='rgba(255,255,255,0.08)'; }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M23.5 6.2a3 3 0 00-2.1-2.1C19.5 3.5 12 3.5 12 3.5s-7.5 0-9.4.6A3 3 0 00.5 6.2C0 8.1 0 12 0 12s0 3.9.5 5.8a3 3 0 002.1 2.1c1.9.6 9.4.6 9.4.6s7.5 0 9.4-.6a3 3 0 002.1-2.1C24 15.9 24 12 24 12s0-3.9-.5-5.8zM9.7 15.5V8.5l6.3 3.5-6.3 3.5z"/></svg>
            </a>
            {/* Instagram */}
            <a href="#" aria-label="Instagram" style={{
              color: 'rgba(232,228,220,0.45)', width: 36, height: 36,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8,
              transition: 'color 0.3s, border-color 0.3s',
            }}
            onMouseEnter={e => { e.currentTarget.style.color='#b8cc8a'; e.currentTarget.style.borderColor='rgba(184,204,138,0.35)'; }}
            onMouseLeave={e => { e.currentTarget.style.color='rgba(232,228,220,0.45)'; e.currentTarget.style.borderColor='rgba(255,255,255,0.08)'; }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.2c3.2 0 3.6 0 4.9.1 3.3.1 4.8 1.7 4.9 4.9.1 1.3.1 1.6.1 4.8 0 3.2 0 3.6-.1 4.8-.1 3.2-1.7 4.8-4.9 4.9-1.3.1-1.6.1-4.9.1-3.2 0-3.6 0-4.8-.1-3.3-.1-4.8-1.7-4.9-4.9C2.2 15.6 2.2 15.2 2.2 12c0-3.2 0-3.6.1-4.8C2.4 3.9 4 2.3 7.2 2.3c1.2-.1 1.6-.1 4.8-.1zM12 0C8.7 0 8.3 0 7.1.1 2.7.3.3 2.7.1 7.1 0 8.3 0 8.7 0 12c0 3.3 0 3.7.1 4.9.2 4.4 2.6 6.8 7 7C8.3 24 8.7 24 12 24c3.3 0 3.7 0 4.9-.1 4.4-.2 6.8-2.6 7-7 .1-1.2.1-1.6.1-4.9 0-3.3 0-3.7-.1-4.9C23.7 2.7 21.3.3 16.9.1 15.7 0 15.3 0 12 0zm0 5.8a6.2 6.2 0 100 12.4A6.2 6.2 0 0012 5.8zM12 16a4 4 0 110-8 4 4 0 010 8zm6.4-11.8a1.4 1.4 0 100 2.8 1.4 1.4 0 000-2.8z"/></svg>
            </a>
          </div>
        </div>

        {/* Center — Quick Links */}
        <div style={{ flex: '1 1 180px' }}>
          <div style={{
            fontSize: '0.65rem', fontWeight: 500,
            letterSpacing: '0.25em', textTransform: 'uppercase',
            color: '#b8cc8a', marginBottom: '1.2rem',
          }}>Quick Links</div>
          <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
            {links.map(([label, to]) => (
              <li key={to} style={{ marginBottom: '0.6rem' }}>
                <Link to={to} style={{
                  fontSize: '0.85rem', fontWeight: 300,
                  color: 'rgba(232,228,220,0.45)',
                  textDecoration: 'none', transition: 'color 0.25s',
                }}
                onMouseEnter={e => e.currentTarget.style.color='#b8cc8a'}
                onMouseLeave={e => e.currentTarget.style.color='rgba(232,228,220,0.45)'}
                >{label}</Link>
              </li>
            ))}
          </ul>
        </div>

        {/* Right — Contact Info */}
        <div style={{ flex: '1 1 220px' }}>
          <div style={{
            fontSize: '0.65rem', fontWeight: 500,
            letterSpacing: '0.25em', textTransform: 'uppercase',
            color: '#b8cc8a', marginBottom: '1.2rem',
          }}>Contact Info</div>
          <div style={{ fontSize: '0.85rem', fontWeight: 300, color: 'rgba(232,228,220,0.45)', lineHeight: 1.8 }}>
            <a href="https://maps.google.com/?q=Vishwakarma+Institute+of+Technology+Bibwewadi+Pune" target="_blank" rel="noopener noreferrer" style={{ color:'rgba(232,228,220,0.45)', textDecoration:'none', transition:'color 0.25s', display:'block' }}
            onMouseEnter={e => e.currentTarget.style.color='#b8cc8a'}
            onMouseLeave={e => e.currentTarget.style.color='rgba(232,228,220,0.45)'}
            >VIT Pune</a>
            <a href="mailto:abhivirddhi@gmail.com" style={{ color:'rgba(232,228,220,0.45)', textDecoration:'none', transition:'color 0.25s', display:'block', marginBottom:'1.2rem' }}
            onMouseEnter={e => e.currentTarget.style.color='#b8cc8a'}
            onMouseLeave={e => e.currentTarget.style.color='rgba(232,228,220,0.45)'}
            >abhivirddhi@vit.edu</a>
          </div>
          <Link to="/contact" style={{
            display: 'inline-block',
            fontFamily: "'DM Sans', sans-serif",
            fontSize: '0.75rem', fontWeight: 500,
            letterSpacing: '0.08em',
            color: '#0a0d0f', background: '#b8cc8a',
            padding: '9px 22px', borderRadius: 8,
            textDecoration: 'none',
            transition: 'background 0.3s',
          }}
          onMouseEnter={e => e.currentTarget.style.background='#cde09e'}
          onMouseLeave={e => e.currentTarget.style.background='#b8cc8a'}
          >Contact Us</Link>
        </div>
      </div>

      {/* Bottom bar */}
      <div style={{
        borderTop: '1px solid rgba(255,255,255,0.05)',
        padding: '1.4rem 0',
        display: 'flex',
        justifyContent: 'space-between',
        alignItems: 'center',
        flexWrap: 'wrap',
        gap: '0.5rem',
        fontSize: '0.72rem', fontWeight: 300,
        color: 'rgba(232,228,220,0.2)',
        letterSpacing: '0.05em',
      }}>
        <span>© 2026 Abhivriddhi Academic Club. All rights reserved.</span>
        <span>Made by Abhivriddhi Technical Team 2026</span>
      </div>
    </footer>
  );
}
